...
function Header()
{
  ...
  //$this->Cell(40,5,'Attendence Report',1,0,'C');
	...
	$this->Ln(4);
		$this->Cell(190,5,'Debtor List',0,0,'C');
	  $this->Ln(2);
	  $this->SetFont('Arial','',12);
...
$this->CellFitScale(10,10,"SL",1,0,"C",true);
$this->CellFitScale(70,10,"Name",1,0,"C",true);
$this->CellFitScale(40,10,"Proprietor Name",1,0,"C",true);
$this->cell(40,10,"Phone",1,0,"C",true); 
$this->cell(40,10,"Due Payment",1,1,"C",true);
...
{
	$sql69="select * from `debtors` where name='".$row['name']."' ";  
	$query69=mysql_query($sql69);
	while($row69=mysql_fetch_array($query69))
	{
		$amount=$amount+$row69['amount'];
	}
	 $sql23="select distinct invoice_date,invoice_no,grand_total from `product_out` where `sale_by` ='".$row['name']."'";  
	 $query23=mysql_query($sql23);
	while($row23=mysql_fetch_array($query23))
	{
		$amount=$amount+$row23['grand_total'];
	}
	$sql27="select * from debtors_receive where name='".$row['name']."'";   
   	$query27=mysql_query($sql27);
	while($row27=mysql_fetch_array($query27))
	{
		$amount=$amount-$row27['amount'];
	}
	$sql23="select * from debtors_receive where name='".$row['name']."' and `cat`='Advance'";   
   	$query23=mysql_query($sql23);
	while($row23=mysql_fetch_array($query23))
	{
...
$this->CellFitScale(10,10,$i,'TLRB',0,"C");
$this->CellFitScale(70,10,' '.$row['name'],'TLRB',"C");
$this->CellFitScale(40,10,' '.$row['debtors_id'],'TLRB',0,"C");
$this->CellFitScale(40,10,' '.$row['phone'],'TLRB',0,"C"); 
$this->CellFitScale(40,10,' '.$amount,'TLRB',1,"C");
...
$this->cell(160,10,"Total",'TLRB',0,"R");
$this->SetFont('Arial','B',12);
$this->CellFitScale(40,10,' '.$total,'TLRB',1,"C");
