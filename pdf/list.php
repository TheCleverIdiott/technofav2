...
while($row=mysql_fetch_array($result))
{
	...
	while($row69=mysql_fetch_array($query69))
	{
	 ...
	 $sql23="select distinct invoice_date,invoice_no,total from `sale` where `sale_by` ='".$row['name']."'";  
	...
	while($row23=mysql_fetch_array($query23))
	{
		$amount=$amount+$row23['total'];
		 ...
		 $sql23="select distinct invoice_date,invoice_no,total from `raw_material_sale` where `sale_by` ='".$row['name']."'";  
	 $query23=mysql_query($sql23);
	while($row23=mysql_fetch_array($query23))
	{
		$amount=$amount+$row23['total'];
...
$this->CellFitScale(30,10,' '.$row['debtors_id'],'TLRB',0,"C");
